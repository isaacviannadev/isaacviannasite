...
const borderSvg = (w, h, r) =>
  Buffer.from(
    `<svg width="${w}" height="${h}"><rect x="0.75" y="0.75" width="${w - 1.5}" height="${h - 1.5}" rx="${r}" ry="${r}" fill="none" stroke="rgba(243,239,231,0.18)" stroke-width="1.5"/></svg>`,
  );

const shadowSvg = (w, h, r, pad, alpha) =>
  Buffer.from(
    `<svg width="${w + 2 * pad}" height="${h + 2 * pad}"><rect x="${pad}" y="${pad}" width="${w}" height="${h}" rx="${r}" ry="${r}" fill="rgba(0,0,0,${alpha})"/></svg>`,
  );
...
async function make(inPath, outName, ar, opts = {}) {
  ...
  const r = Math.max(
    ...
    Math.min(
      tight ? 14 : 22,
      Math.round(Math.min(sw, sh) * (tight ? 0.022 : 0.035)),
    ),
  );
    ...
    .composite([
      {
        ...
        top: Math.max(0, top - pad + (tight ? 16 : 26)),
      },
      { input: shot, left, top },
    ])
  ...
  console.log(
    "ok",
    outName,
    `${W}x${H}  (shot ${sw}x${sh})${tight ? "  [tight]" : ""}`,
  );
}
...
for (const [inPath, outName, ar, opts] of jobs) {
  await make(inPath, outName, ar, opts).catch((e) =>
    console.error("FAIL", outName, e.message),
  );
}
